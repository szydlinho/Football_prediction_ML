...
def PrepareDataset(league_abb, league):
    ...
    if (len(dataset["raw_data_10"])% 10 == 0):
        number_of_MW = 10
    elif(len(dataset["raw_data_10"])% 9 == 0):
        number_of_MW = 9
    elif(len(dataset["raw_data_10"])% 12 == 0):
        number_of_MW = 12
    ...
    #fixtures = table[["Date", "Home team", "Away team"]]
...
#             elo_new_teams = elo.loc[elo["Country"] == "GER"]["Club"].unique().tolist()   
...
#     list(set(elo_teams))
     ...
     marketv = pd.read_csv(loc + 'budget_'+ league_abb +'_final.csv', index_col = 0)
     ...
     dataset = pd.merge(final_df, marketv,  how='inner', left_on=['AwayTeam','Sezon'], 
     ...
     to_predict = df_final.loc[(df_final['Sezon']==20) & (df_final['MW']==4)]
     training_dataset =  df_final.loc[~((df_final['Sezon']==20) & (df_final['MW']==4))]
